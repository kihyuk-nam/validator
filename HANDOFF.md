...
**② 드물게 `502 Bad Gateway` 가 난다 — 재시도하면 된다.**
무료 인스턴스에서 실제로 관측된 현상이다. Render 의 프록시가 내는 응답이고 우리 서버 코드와는 무관하다(인스턴스 재시작·순간적 프록시 장애). 확인된 범위는 이렇다.

- 재현되지 않는 일회성이었다. 직후 `/healthz` 5회 연속 200(0.2초), 통합 테스트 PASS, `compare_specs.py` 4회 연속(총 56회 호출) 전부 정상.
- 그래도 **자동화·배치에서는 502·503 에 재시도를 넣을 것.** 지수 백오프로 2~3회면 충분하다. `4xx` 는 입력 문제이므로 재시도해도 같은 결과다 — `5xx` 만 재시도한다.
- 단발 호출이면 그냥 한 번 더 부르면 된다.
